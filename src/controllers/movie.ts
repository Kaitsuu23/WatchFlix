...
/**
 * Controller for `/filter` route
 * Supports: sort, type, genre, country, year, page
 */
export const filterContent: TController = async (req, res) => {
    try {
        const { sort = 'populer', type = 'movie', genre = '', country = '', year = '', page = 0 } = req.query;

        let path = `/${sort}`;
        if (type)    path += `/type/${type}`;
        if (genre)   path += `/genre1/${genre}`;
        if (country) path += `/country/${country}`;
        if (year)    path += `/tahun/${year}`;
        if (Number(page) > 1) path += `/page/${page}`;

        const axiosRequest = await axios.get(`${process.env.LK21_URL}${path}`);
        const payload = await scrapeMovies(req, axiosRequest);
        res.status(200).json(payload);
    } catch (err) {
        console.error(err);
